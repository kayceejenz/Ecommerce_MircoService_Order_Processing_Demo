...
```
               ...
               REST/HTTP2   │  REST    │  REST    │
                 ...
                 │  (:5010)    │  │  (:5020) │  │ (:5040)      │
                 │             │  │          │  │               │
                 │ REST+gRPC   │  │ CQRS+ES  │  │ Event-Driven  │
                 │ PG+Redis    │  │ Saga     │  │ (simulated)   │
                 └──────┬──────┘  └──┬───┬──┘  └───────┬───────┘
                        │           │   │               │
              gRPC      │    Events │   │ Events        │ Events
              Streaming │   (RabbitMQ)  │ (RabbitMQ)    │ (RabbitMQ)
                        │           │   │               │
                 ┌──────▼──┐   ┌───▼───▼───────────────▼──────┐
                 │Inventory│   │        RabbitMQ               │
            ...
            (RabbitMQ)  │          │               │
                        │          ▼               ▼
                 ┌──────▼──────────────────────────────────┐
                 │         Notification Service (:5050)     │
                 │    (Event Consumer + gRPC Streaming)     │
                 └─────────────────────────────────────────┘
```
